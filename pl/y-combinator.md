...
我们来看fac-gen的形式，其等价于<code>(defn fac-gen [f] (if (<= n 0) 1 (* n (f (dec n)))))</code>，是不是和我们理想中的fact形式已经很接近了，当然其中有一个自由变量n，没关系，我们给他加个帽子。
~~~clojure
(defn fact5 [self]
  (fn [n]
    (letfn [(fac-gen [f]
...
这次的fac-gen和我们的理想函数形式非常像了，换成二元就是<code>(defn fac-gen [f, n] (if (<= n 0) 1 (* n (f (dec n)))))</code>，这正是我们找寻的理想形式。
(defn fact6 [self]
  (letfn [(fac-gen [f]
            (fn [n] (if (<= n 0) 1 (* n (f (dec n))))))]
